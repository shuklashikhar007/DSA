// same code and buy and sell 2 just subtract the fee from either the buy branch or the sell branch
...
int f(int ind, int buy, vector<int>&arr, vector<vector<int>> &dp){
    ...
    if(buy){ // agar uss index par stock buy kiya to
         ...
         // without fee upar wala code hota
          // with fee ye wala code hogaprofit = max(-arr[ind] + f(ind+1,0,arr,dp) - fee, f(ind+1,1,arr,dp));
    }
...
int main(){
    ...
    // Yaha function mai fee pass kardena 
    int profit = R(arr);
    cout << profit <<  endl;
    return 0;
}