...
// Cooldown means we cannot sell right after buying a stock so simply replace ind+1 to ind+2 in the sell branch of recursion tree
#include<bits/stdc++.h>
using namespace std;
int f(int ind, int buy, vector<int>&arr, vector<vector<int>> &dp){
    ...
    if(buy){ // agar uss index par stock buy kiya to
        ...
        profit = max(arr[ind] + f(ind+2, 1,arr,dp), 0 + f(ind+1,0,arr,dp));
    }
